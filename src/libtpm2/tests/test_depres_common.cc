...
public:
	...
	{
		G.insert(make_pair(make_pair(name, arch), make_shared<IGNode>(*this, make_pair(name, arch), true, false)));
	}
...
BOOST_AUTO_TEST_CASE( test_ig_node_set_dependencies_set )
{
	...
	BOOST_TEST(v->dependencies[0] == w.get());
	BOOST_TEST(*w->reverse_dependencies.begin() == v.get());
}
...
BOOST_AUTO_TEST_CASE( test_ig_node_set_dependencies_modify )
{
	...
	BOOST_TEST(v->dependencies[0] == u.get());
	BOOST_TEST(*u->reverse_dependencies.begin() == v.get());
}
